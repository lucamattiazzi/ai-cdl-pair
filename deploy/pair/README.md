# Self-host AI-CDL Pair

The deployment runs the Pair task pane and authenticated WebSocket relay behind Caddy. Caddy obtains
...
`PAIR_DOMAIN` is the deployment setting, not a domain compiled into the product. Set it to your own
...
adapter. No shared registry, account or dependency on another Pair operator is required.

## VM prerequisites

- A Linux VM with Docker Engine and the Compose plugin.
- An `A`/`AAAA` DNS record pointing the Pair domain to the VM.
...
cd deploy/pair
cp .env.example .env
# Set PAIR_DOMAIN in .env, then:
docker compose up --build -d
docker compose ps
docker compose exec pair node -e "fetch(process.env.PAIR_PUBLIC_ORIGIN + '/healthz').then(r => { if (!r.ok) process.exitCode = 1; return r.text(); }).then(console.log)"
```
...
The Docker build already generates and serves `/manifest.xml` for `PAIR_DOMAIN`. Download it from
your instance and sideload it in Excel. For a separate source build, generate with the same origin:

```sh
PAIR_PUBLIC_ORIGIN=https://pair.example.com \
  pnpm --filter @ai-cdl/pair-addin manifest:production
```

Validate and sideload `apps/pair-addin/manifest.xml` before submitting it to Microsoft AppSource.

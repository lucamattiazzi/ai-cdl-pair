# AI-CDL Pair Session and RPC 0.2

## Persistent session

Start a remembered bridge with `node scripts/session.mjs start --name desk`. For the first start
only, pass the private setup URL in `AI_CDL_PAIR_URL`. Add `--name desk` to each subsequent command;
...
`--session` or `AI_CDL_PAIR_SESSION` remain available for scripts using explicit control paths.

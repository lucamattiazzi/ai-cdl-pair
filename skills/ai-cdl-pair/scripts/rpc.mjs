...
const response = await new Promise((resolve, reject) => {
  ...
  const timer = setTimeout(() => {
    socket.close();
    rejectOnce(new Error(`AI-CDL Pair request timed out after ${timeoutMs}ms.`));
  }, timeoutMs);
  ...
  socket.addEventListener("error", () => {
    rejectOnce(new Error("Could not connect to the AI-CDL Pair URL."));
  });
  socket.addEventListener("close", () => {
    rejectOnce(new Error("AI-CDL Pair connection closed before a response was received."));
  });
});

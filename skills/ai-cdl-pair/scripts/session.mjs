...
const profileDirectory = process.env.AI_CDL_PAIR_HOME ?? join(homedir(), ".ai-cdl-pair");
...
function pairUrl() {
  const url = process.env.AI_CDL_PAIR_URL ?? readProfile(profileName())?.url;
  if (!url) fail("AI_CDL_PAIR_URL is required.");
  let parsed;
  try {
    parsed = new URL(url);
  } catch {
    return fail("AI_CDL_PAIR_URL must be a valid WebSocket URL.");
  }
  if (parsed.protocol !== "ws:" && parsed.protocol !== "wss:") {
    fail("AI_CDL_PAIR_URL must use ws:// or wss://.");
  }
...
function controlPathForNewSession() {
  if (process.platform === "win32") return `\\\\.\\pipe\\ai-cdl-pair-${randomUUID()}`;
  const directory = mkdtempSync(join(tmpdir(), "ai-cdl-pair-"), { mode: 0o700 });
...
async function runDaemon(controlPath) {
  ...
  if (!url) fail("The Pair URL was not provided on stdin.");
  ...
  const closedMessage = () =>
    closeInfo?.reason
      ? `Pair connection closed (${closeInfo.code}): ${closeInfo.reason}`
      : `Pair connection closed${closeInfo ? ` (code ${closeInfo.code})` : ""}.`;
...
function send(controlPath, payload, timeoutMs = 3_600_000) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      connection.destroy();
      reject(new Error("The AI-CDL Pair session did not respond."));
    }, timeoutMs);
    connection.on("close", () => {
      clearTimeout(timer);
      reject(new Error("The Pair bridge closed the local request without a response."));
    });
    connection.on("error", () => {
      clearTimeout(timer);
      reject(new Error("No running AI-CDL Pair session. Run `session.mjs start` first."));
    });
    ...
    reader.once("line", (line) => {
      ...
      try {
        ...
        reject(new Error("The AI-CDL Pair session returned an invalid response."));
      }
    });
  });
}
...
async function start() {
  ...
  const handshake = await new Promise((resolve, reject) => {
    const timer = setTimeout(
      () => reject(new Error("Starting the Pair session timed out.")),
      30_000,
    );
    ...
    reader.once("line", (line) => {
      ...
      try {
        ...
        reject(new Error("The Pair session produced an unreadable handshake."));
      }
    });
    ...
    child.once("exit", () => {
      clearTimeout(timer);
      reject(new Error("The Pair session exited before it was ready."));
    });
  });
  if (!handshake.ok) fail(handshake.error ?? "The Pair session could not start.");
...
async function main() {
  ...
  const controlPath = argument(
    "--session",
    name ? readProfile(name)?.session : process.env.AI_CDL_PAIR_SESSION,
  );
  if (!controlPath) fail("Set AI_CDL_PAIR_SESSION or pass --session with the path from `start`.");

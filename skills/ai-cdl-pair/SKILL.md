---
name: ai-cdl-pair
description: Connect a remembered terminal to the Excel AI-CDL Pair task pane, receive its chat messages, inspect bounded workbook ranges and preview or commit changes through Excel approval. Use when the user asks to pair Excel, reconnect a Pair terminal, or work in their open workbook using Pair.
license: Apache-2.0
metadata:
  author: ai-cdl
  version: "0.3.0"
...
When `ai-cdl-pair-agent` is installed, use its `list` and `status --name NAME` commands to discover
saved profiles. The user can start `ai-cdl-pair-agent codex|opencode|claude --name NAME` locally and
...
profile, reuse its name; when several exist, let the user choose. A profile belongs to a Pair bridge,
...
`start` only, supply its URL as `AI_CDL_PAIR_URL` in the command environment and choose a recognizable
`--name`. Avoid printing the URL. The bridge intentionally stores the credential in an owner-only
file under `~/.ai-cdl-pair`; never store it in source, logs, traces, commits or chat replies. Prefer a

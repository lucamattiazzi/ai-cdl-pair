...
export function generateManifest(project: LoadedProject, environment: ManifestEnvironment): string {
  ...
  groupIcons.up();
  const control = group.ele("Control", { "xsi:type": "Button", id: "AiCdl.Open" });
...
export async function writeManifest(
  ...
  if (check) {
    ...
    if (current !== generated)
      throw new Error(
        `AI_CDL_MANIFEST_DRIFT: ${target} differs. Run manifest generate without --check and commit the result.`,
      );
    return target;
  }
...
export async function validateManifestInputs(
  ...
  if (environment === "production" && new URL(url).protocol !== "https:")
    errors.push(
      "AI_CDL_MANIFEST_HTTPS_REQUIRED: Production task-pane URLs must use HTTPS. Fix taskpane.productionUrl.",
    );
  for (const asset of [
    ...
    try {
      ...
      errors.push(`AI_CDL_MANIFEST_ICON_MISSING: Referenced icon does not exist: ${asset}.`);
    }
  }
  return errors;
}

...
const program = new Command()
  .name("ai-cdl")
  .description("Generate and package safe AI-CDL Excel integrations.")
  .version("0.1.0");
const invocationDirectory = process.env.INIT_CWD ?? process.cwd();

program
  .command("init")
  .description("Create a minimal typed AI-CDL config in the current directory.")
  .action(async () => {
    const target = resolve(invocationDirectory, "ai-cdl.config.ts");
    try {
      await access(target);
      throw new Error(
        "AI_CDL_INIT_EXISTS: ai-cdl.config.ts already exists; no files were overwritten.",
      );
    } catch (error) {
    ...
    console.log(`Created ${target}. Review URLs and run ai-cdl-office doctor.`);
  });
...
async function createDeploymentPackage(): Promise<string> {
  ...
  const directory = resolve(project.root, "dist/ai-cdl-addin");

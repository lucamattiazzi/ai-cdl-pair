import { type JsonValue, toolDescriptorRegistry } from "@ai-cdl/protocol";
...
If the workbook disconnects, stop mutations and ask the user to reconnect the saved profile. The Pair association identifies the bridge; it does not authorize a different workbook silently.`;
...
export function createExcelMcpServer(call: BridgeCall, channel = false): Server {
  const server = new Server(
    { name: "ai-cdl-pair", version: "0.1.1" },
    {
      capabilities: {
        tools: {},
        resources: {},

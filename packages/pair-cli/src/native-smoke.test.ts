...
import { createPairIdentity, pairConnectionUrl } from "@ai-cdl/transport";
...
it.skipIf(process.env.PAIR_NATIVE_SMOKE !== "1")(
  ...
  async () => {
    ...
    const environment = { AI_CDL_PAIR_HOME: dir };
    const invoke = (command: string, url?: string) =>
      promisify(execFile)(
        process.execPath,
        ["skills/ai-cdl-pair/scripts/session.mjs", command, "--name", "desk"],
        { env: { ...process.env, ...environment, ...(url ? { AI_CDL_PAIR_URL: url } : {}) } },
      );
    ...
    try {
      ...
      const command = [
        process.execPath,
        resolve("packages/pair-cli/dist/agent.js"),
        "mcp",
        "--name",
        "desk",
      ];

...
async function pathForBridge(): Promise<string> {
  ...
  if (
    ...
    throw new Error("Adapters require an encrypted Pair profile.");
...
async function main() {
  ...
  if (command === "pair") {
    ...
    process.env.AI_CDL_PAIR_URL = url;
    try {
      await invoke("start");
    } finally {
      delete process.env.AI_CDL_PAIR_URL;
    }
    console.log(
      `Saved ${name}. Run ai-cdl-pair-agent codex|opencode|claude --name ${name}, then reconnect in Excel.`,
    );
    return;
  }
  ...
  const environment = { AI_CDL_PAIR_HOME: profileDirectory() };
...
void main().catch((error: unknown) => {
  console.error(error instanceof Error ? error.message : "Pair adapter failed.");
  process.exitCode = 1;
});

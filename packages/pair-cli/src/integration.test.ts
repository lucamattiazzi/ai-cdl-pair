...
import { createPairAddinSession } from "@ai-cdl/pair";
import {
...
import { z } from "zod";
import { createPairServer } from "../../../apps/pair-server/src/index.js";
...
it("runs MCP reads, approved writes/charts and independent private traces over the encrypted bridge", async () => {
  ...
  const env: NodeJS.ProcessEnv = { ...process.env, AI_CDL_PAIR_HOME: dir };
  delete env.AI_CDL_PAIR_URL;
  const invoke = (args: string[], url?: string) =>
    promisify(execFile)(
      process.execPath,
      ["skills/ai-cdl-pair/scripts/session.mjs", ...args, "--name", "desk"],
      { env: { ...env, ...(url ? { AI_CDL_PAIR_URL: url } : {}) } },
  ...
  try {
    ...
    await channelClient.connect(
      new StdioClientTransport({
        ...
        args: [resolve("packages/pair-cli/dist/agent.js"), "mcp", "--name", "desk", "--channel"],
        env: {
          ...Object.fromEntries(

import type { AddinController } from "@ai-cdl/addin-core";
import {
...
import type { Transport, TransportUnsubscribe } from "@ai-cdl/transport";
...
/** Build a validated chat event for one Pair role. */
...
export interface PairAddinSession extends PairLifecycle {
  ...
  /** Observe agent replies delivered over the Pair session. */
...
/** Typed RPC client used by local Pair agents. */
export function createPairClient(options: PairClientOptions): PairClient {
  ...
  function receive(message: Parameters<Transport["send"]>[0]): void {
    ...
    if (message.type === "error") request.reject(new PairProtocolError(message.error));
      ...
      request.reject(new Error("Pair response method does not match the request."));
  ...
  return {
    async connect() {
      ...
      unsubscribeState = options.transport.subscribeState((state) => {
        if (state === "disconnected")
          rejectPending(
            "Pair transport disconnected. Check the workbook before retrying a mutation.",
          );
      });
      try {
        await options.transport.connect();
    ...
    request(method, params) {
      if (options.transport.state !== "connected") {
        return Promise.reject(new Error("Pair client is not connected."));
      }
      const id = createId();
      ...
      return new Promise((resolve, reject) => {
        const timer = setTimeout(() => {
          pending.delete(id);
          reject(new Error(`Pair request timed out: ${id}.`));
        }, timeoutMs);
        pending.set(id, {
          method,
    ...
    async close() {
      ...
      unsubscribeState?.();
      unsubscribeState = undefined;
      rejectPending("Pair client closed.");
      chatListeners.clear();
      await options.transport.close();
    },
  };

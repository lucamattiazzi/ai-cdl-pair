...
import { PROTOCOL_VERSION, parseProtocolMessage } from "@ai-cdl/protocol";
...
/** Create the production Pair static host and authenticated WebSocket rendezvous relay. */
export async function createPairServer(options: PairServerOptions): Promise<PairServer> {
  ...
  const bridgeScript = resolve(options.bridgeScript ?? "skills/ai-cdl-pair/scripts/session.mjs");
    ...
    socket.on("message", (data: RawData, isBinary: boolean) => {
      ...
      try {
        ...
        if (message.type === "event" && message.event === "pair.peer.changed") {
          throw new Error("Pair presence events are server-owned.");
        }
        const peer = session[peerRole];
    ...
    void (async () => {
      ...
      if (request.method === "POST" && url.pathname === "/api/pair/sessions") {
        ...
        if (sessions.size >= maxActiveSessions) {
          json(response, 503, { error: "Pair relay capacity reached. Try again later." });
          return;
        }
  ...
  if (!address || typeof address === "string")
    throw new Error("Pair server address is unavailable.");

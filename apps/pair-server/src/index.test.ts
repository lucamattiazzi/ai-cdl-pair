...
import { PROTOCOL_VERSION } from "@ai-cdl/protocol";
...
describe("Pair server", () => {
  ...
  it("serves the agent bridge script the pairing response points at", async () => {
    server = await createPairServer({
      ...
      bridgeScript: fileURLToPath(
        new URL("../../../skills/ai-cdl-pair/scripts/session.mjs", import.meta.url),
      ),
    });
    const pairing = (await (
  ...
  it("serves the add-in with restrictive browser headers", async () => {
    const directory = await mkdtemp(join(tmpdir(), "ai-cdl-pair-"));
  ...
  it("keeps one agent connection for task-pane chat and Excel work", async () => {
    ...
    const run = (args: readonly string[], session?: string) =>
      execFileAsync(process.execPath, [script, ...args], {
        env: {
          ...process.env,
          AI_CDL_PAIR_URL: local(pairing.agentUrl),
          ...(session ? { AI_CDL_PAIR_SESSION: session } : {}),
        },
  ...
  it("fails a second bridge start with the reason the relay gave", async () => {
    ...
    const start = () =>
      execFileAsync(process.execPath, [script, "start"], {
        env: { ...process.env, AI_CDL_PAIR_URL: local(pairing.agentUrl) },
      });
    ...
    try {
      ...
      await execFileAsync(process.execPath, [script, "stop"], {
        env: { ...process.env, AI_CDL_PAIR_SESSION: first.session },
      }).catch(() => undefined);
      addin.close();
    }
  }, 30_000);
  ...
  it("supports the standalone Agent Skill RPC client", async () => {
    ...
    const { stdout } = await execFileAsync(
      process.execPath,
      [
        fileURLToPath(new URL("../../../skills/ai-cdl-pair/scripts/rpc.mjs", import.meta.url)),
        "--method",
        "excel.sheet.list",
        "--params",
        "{}",
      ],
      { env: { ...process.env, AI_CDL_PAIR_URL: localUrl(pairing.agentUrl) } },
    );

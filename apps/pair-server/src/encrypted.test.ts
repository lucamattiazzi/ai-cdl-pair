...
it("remembers a named bridge and reconnects without another URL", async () => {
  ...
  const env: NodeJS.ProcessEnv = { ...process.env, AI_CDL_PAIR_HOME: directory };
  delete env.AI_CDL_PAIR_URL;
  delete env.AI_CDL_PAIR_SESSION;
  const script = "skills/ai-cdl-pair/scripts/session.mjs";
  const invoke = (args: string[], pairUrl?: string) =>
    run(process.execPath, [script, ...args], {
      env: { ...env, ...(pairUrl ? { AI_CDL_PAIR_URL: pairUrl } : {}) },

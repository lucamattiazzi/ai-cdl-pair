...
Production static host and WebSocket rendezvous relay for the free Pair add-in.

Required environment:

- `PAIR_PUBLIC_ORIGIN`: public HTTPS origin, for example `https://pair.example.com`.
...
- `PAIR_BRIDGE_SCRIPT`: optional path to the agent session bridge served at `/agent/session.mjs`.
  Defaults to `skills/ai-cdl-pair/scripts/session.mjs` relative to the working directory.
...
- `PAIR_STATIC_DIR`: task pane build directory; defaults to `apps/pair-addin/dist`.

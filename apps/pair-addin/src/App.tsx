...
import { createPairAddinSession, type PairAddinSession } from "@ai-cdl/pair";
import { VirtualWorkbookDriver } from "@ai-cdl/testing";
import {
...
function DirectChat({ settings, disconnect }: DirectChatProps) {
  ...
  useEffect(() => {
    ...
    return () => {
      unsubscribe();
      session.cancel("Disconnected from Pair.");
    };
  }, [session]);
  ...
  const createPairing = useCallback(
    async (existing?: SavedTerminal, name = "My terminal"): Promise<void> => {
      ...
      try {
        ...
        abortFetch.current = abort;
        const relayOrigin = import.meta.env.VITE_PAIR_RELAY_ORIGIN || window.location.origin;
        let terminal = existing;
        if (!terminal) {
          const response = await fetch(new URL("/api/config", relayOrigin), {
            signal: abort.signal,
          });
          if (!response.ok) throw new Error(`Pair server returned HTTP ${response.status}.`);
  ...
  return (
    <main>
      ...
      <section className="relay-panel">
        ...
        <p>
          Use your existing agent in OpenCode, Codex or Claude Code. Pair remembers the terminals
          you authorize.
        </p>
        <div className="privacy-note">
          End-to-end encrypted · Pair relays cannot read workbook data or conversations.
        </div>
        {terminals.length > 0 && (
          <details className="connection-settings" open={!agentConnected}>
            ...
            <section className="terminal-list" aria-label="Authorized terminals">
              ...
              <p className="fine-print">
                Start your saved Pair adapter, then reconnect here. One workbook per terminal
                connection.
              </p>
            </section>
          </details>
        ...
        {pairing &&
          ...
          !terminals.some((record) => record.id === pairing.terminal.id) && (
            <div className="pairing-ticket">
              ...
              <p>
                Run ai-cdl-pair-agent pair --name desk in your terminal and paste the private URL
                below. Then start your harness adapter. You only need this URL once.
              </p>
              <button
                className="primary"
                type="button"
                onClick={() => {
                  void navigator.clipboard.writeText(pairing.agentUrl).then(
                    () => setStatus("Private URL copied. Paste it into the Pair setup command."),
  ...
  return (
    <main className="onboarding">
      <header className="hero">
        ...
        <p>
          Pair connects an agent you trust to the workbook in front of you. Reads are bounded.
      ...
      <form className="connect-card" onSubmit={connectDirect}>
        ...
        <div>
          ...
          <p>Best for an HTTPS agent endpoint that supports AI-CDL protocol 0.1 and CORS.</p>
        </div>
      ...
      <footer>Open protocol · Approval before writes · No agent credentials on Pair servers</footer>
    </main>
  );
}
